...
(function($){
    ...
    $.addFlex = function(t,p)
    {
        ...
        p = $.extend({
            ...
            rpOptions: [10,15,20,25,40,100,1000],
            title: false,
        ...
        var g = {
            ...
            addData: function (data) { //parse data
                ...
                if (p.dataType=='json')
                {
                    parseResponseData(data, g, tbody, p);
                //					$.each
                //					(
                //					 data.rows,
                //					 function(i,row)
                ...
                //										td.align = this.align;
                //                                                                                if(p.colModel[idx].formatter){
                ////                                                                                    td.innerHTML = p.colModel[idx].formatter;
                //                                                                                    p.colModel[idx].formatter(td, row.cell[idx]);
                //                                                                                }else{
            ...
            populate: function () { //get latest data
                ...
                for (x in p.userSearch)
                {
                    param.push(p.userSearch[x]);
                }
                $.ajax({
                    type: p.method,
                    url: p.url,
        ...
        if (p.colModel)
        {
            ...
            for (i=0;i<p.colModel.length;i++)
            {
                ...
                //alert(cm.display);
                //if(cm.display!=undefined || cm.display)
                th.innerHTML = cm.display;
					
                //if (cm.name&&cm.sortable)
                //$(th).attr('abbr',cm.name);
                if (cm.sortCol&&cm.sortable)
                    $(th).attr('abbr',cm.sortCol);
        ...
        if (p.usepager)
        {
            ...
            if (p.searchitems)
            {
                ...
                if(p.displaySearch) {
                    $('.pDiv2').prepend("<div class='pGroup'> <div class='pSearch pButton'><span></span></div> </div>  <div class='btnseparator'></div>");
                }
...
function parseResponseData(data, g, tbody, p){
    var colName = "";
    var firstParam = "";
    ...
    for(j=0; j<data.rows.length; j++){
        ...
        for(i=0; i<p.colModel.length; i++){
            colName = p.colModel[i].name;

            var td = document.createElement('td');
            //td.align = this.align;
            //            td.setAttribute("align", this.align);

            if(p.colModel[i].formatter){
                td.innerHTML = p.colModel[i].formatter;
                p.colModel[i].formatter(td, row);
                
            }else{
                params = new Array();
                params = colName.split('.');
                if(params.length > 1){
                    firstParam = params[0];
                    if((typeof row[firstParam]).indexOf('object') != -1){
                        td.innerHTML = getValue(row, params[0], params, 0);
                    }
                }else
...
var getValue = function (row, colName, params, index){
    if((typeof row[colName]).indexOf('object') != -1&& index < params.lenght){
        index++;
        return getValue(row[colName], params[index], params, index);
    }else
        return row[colName];
}